			...
			<div class="tab2" style="float:left;">
				<ul class="clearfix" style="float:left;">
                    @if(!empty(Request::input('pid')))
                        <li class=""><a href="/usermanager/supps/{{ Request::input('pid') }}">供应商详情</a></li>
                        <li class="cur"><a href="javascript:void(0);">媒体管理</a></li>
                    @else
                        <li class=""><a href="/usermanager/ads_list">供应商管理</a></li>
                        <li class="cur"><a href="javascript:void(0);">媒体管理</a></li>
                    @endif
                ...
                <div style="float:right;">
                    ...
                    <div class="l">
                        <select name="plate_id" class="sel1" id="mediatype">
                            @foreach($media_select as $key => $val)
                                <option value="{{ $val['value'] }}">{{ $val['plate_name'] }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="l">
                        <input type="text" name="username" id="keyword" class="txt5" placeholder="媒体名称、媒体分类" />
    ...
    <tbody id="listcontent">
        @foreach($lists as $key => $value)
            <tr>
                ...
                <td class="color1">{{ $value['proxy_price'] }}</td>
                <td>{{ getSuppUserType($value['is_state']) }}</td>
                <td>
                    <a class="color2" href="/usermanager/suppResourceInfo?id={{ $value['user_id'] }}">
                    {{-- /console/add_view?blade_name=list_4 --}}
                    查看</a>
					<a class="color2" href="/usermanager/sameTypeMedia?id={{ $value['user_id'] }}">同类媒体</a>
                    {{-- /console/add_view?blade_name=list_1 --}}
                </td>
            </tr>
        @endforeach
    </tbody>
