        ...
        <h3 class="title4"><strong><a href="#">活动管理</a></strong>
            <div class="search_1">
                <div style="float:right;">
                    ...
                    <div class="l">
                        <input type="text" name="keyword" id="keyword" class="txt5" placeholder="活动名称" />
                        <input type="submit" name="submit" class="sub4_3" id="searchnews" value="查询" />
                    </div>
                    <div class="l">
                        <a class="sub4_2" href="/console/activity/add" style="background:#7db6eb">创建</a>
                    </div>
                </div>
            </div>
        </h3>
                ...
                <table class="table_in1 cur" id="datatable1">
                    ...
                    <tbody id="listcontent">
                        @foreach($lists as $key => $val)
                            <tr id="activity_{{ $val->id }}">
                                <td>{{ $val->id }}</td>
                                <td>{{ $val->activity_name }}</td>
                                <td>{{ $val->start }}</td>
                                <td>{{ $val->over }}</td>
                                <td>{{ $val->plate_rate }}%</td>
                                <td>{{ $val->vip_rate }}%</td>
                                <td><a class="color2"  href="/console/activity/info/{{ $val->id }}" >活动媒体</a><a class="color2" href="/console/activity/add?id={{ $val->id }}">查看</a>
                                <a class="color1" href="javascript:;" onclick="del_activity({{ $val->id }})">删除</a></td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
...
function del_activity(id) {
    $.ajax({
        data:{"activity_id":id,'_token':$('meta[name="csrf-token"]').attr('content')},
        url:'/console/activity/del',
        dataType:'json',
        type:'post',
