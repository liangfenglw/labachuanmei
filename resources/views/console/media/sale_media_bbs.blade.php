...
<div class="content"><div class="Invoice">
	...
	<div class="main_o clearfix" style="">
		...
		<h3 class="title5 clearfix"><strong>{{$media['plate_name']}}</strong></h3><!--论坛营销-->
			...
			success: function (data) {
				...
				if (data.status == '1') {
					for(var i=0; i< sum; i++){
						...
						data.data[i]['media_name']+'</td><td>'+data.data[i]['publish_type']+'</td>'+
							'<td>'+data.data[i]['channel_type']+'</td><td>'+data.data[i]['appoint_type']+
							'</td><td class="color1">￥'+data.data[i]['proxy_price']+'</td>'+
...
//				user_arr.push($(this).attr('rst_id'));
	...
	jQuery.validator.addMethod("comparetime1", function(value, element) {
		...
		time3 = moment(time2).add(15,"minutes").format("YYYY-MM-DD HH:mm:ss");		//当前时间 +0分钟
